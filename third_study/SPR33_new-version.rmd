...
# Breznau 2020
load(file = wdir("SPR33/cis2.Rdata"))
...
"
configural2 <- cfa(model = meaInmod2, data = issp16_meain)
summary(configural2,standardized = TRUE, fit.measures = TRUE)

configural2.a <- cfa(model = meaInmod2, data = issp16_meain, group = "cow")
fitmeasures(configural2)

metric <- cfa(model = meaInmod2, data = issp16_meain, group = "cow", group.equal = "loadings")
fitmeasures(metric)
...
issp16df$gov_author <- round(rowMeans(select(issp16df, gov_surveil, gov_surveil2, gov_secure), na.rm=T),2)

# alternatively, make an intervention scale 

issp16df$gov_int <- round(rowMeans(select(issp16df, gov_surveil, gov_surveil2, gov_secure,
                                          gov_jobs, gov_prices, gov_health, gov_old,
                                          gov_unemp), na.rm = T))




issp16dfa <- round(aggregate(issp16df, by = list(issp16df$cow), FUN = mean, na.rm = T),2)

issp16dfa <- dplyr::select(issp16dfa, cow, gov_author, gov_int)
...
"
configural4 <- cfa(model = meaInmod4, data = betrack_complete)
configural4b <- cfa(model = meaInmod4, data = betrack_complete, group = "cow", estimator = "GLS")
summary(configural4b)

```


```{r clean_blavatnik, warning = F, message = F}
## turn interger to date
gov_resp <- gov_resp %>%
  mutate(
  ...
  filter(date > "2020-03-15" & date < "2020-03-31" & cow %in% issp16dfa$cow) %>%
  select(CountryName, date, chi)

gov_resp <- aggregate(x = gov_resp[,-2], by = list(gov_resp$CountryName), FUN = mean)
...
testdf <- left_join(issp16dfa, gov_resp, by = "cow")
testdf$discrep <- testdf$chi - testdf$gov_author
```
...
betrack_av <- betrack_av %>% 
    ...
    select(cow, stay, gather, distance, tsymp, hwash)
finaldf_Ca <- filter(finaldf_Ca, cow %in% issp16dfa$cow)
...
m11 <- lm(chi ~ days_since_peak + conf_delta + gov_author , data = df)

m12 <- lm(chi ~ days_since_peak + conf_delta + gov_author + gini_disp, data = df)

m13 <- lm(chi ~ days_since_peak + conf_delta + gov_author + socpolicy, data = df)

m14 <- lm(chi ~ days_since_peak + conf_delta + gov_author + gdp, data = df)

m15 <- lm(chi ~ days_since_peak + conf_delta + gov_author + gini_disp + socpolicy + gdp, data = df)

tab_model(m11, m12, m13, m14, m15, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"),  dv.labels = c("M1", "M2","M3", "M4","M5"), pred.labels = c("Days Since Curve Inflection", "New Case Rate", "Intervention Severity", "Disposable Income Inequality", "Welfare State Strength", "GDP Per Capita"))
```
...
m21 <- lm(stay ~ days_since_peak + conf_delta + discrep, data = df)

m22 <- lm(stay ~ days_since_peak + conf_delta + discrep + gini_disp, data = df)

m23 <- lm(stay ~ days_since_peak + conf_delta + discrep + socpolicy, data = df)

m24 <- lm(stay ~ days_since_peak + conf_delta + discrep + gdp, data = df)

m25 <- lm(stay ~ days_since_peak + conf_delta + discrep + gini_disp + socpolicy + gdp, data = df)

tab_model(m21, m22, m23, m24, m25, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"),  dv.labels = c("M1", "M2","M3", "M4","M5"), pred.labels = c("Days Since Curve Inflection", "New Case Rate", "Intervention Severity", "Disposable Income Inequality", "Welfare State Strength", "GDP Per Capita"),show.std = TRUE)
```
