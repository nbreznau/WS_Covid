...
## For sensitivity analysis: March 20th to March 26th

betrack_sens <- betrack_complete[which(betrack_complete$date > "2020-03-19" & 
                                         betrack_complete$date < "2020-03-27"),]


## Take the Country-Average

betrack_av <- round(aggregate(betrack_complete[,-2], by = list(betrack_complete$cow), FUN = mean, na.rm = T),2)
betrack_av_sens <- round(aggregate(betrack_sens[,-2], by = list(betrack_sens$cow), FUN = mean, na.rm = T),2)

## Now create a standardized score at the country level
betrack_av_sens <- betrack_av_sens %>%
  mutate(
    behave_sens = scale(behave)
  ) %>%
  filter(cow %in% issp16dfa$cow) %>%
  select(cow, behave_sens)
...
##  Outbreak Stage (01/03 - 15/03: 15 days)
...
## Public Reaction Stage (16/03 - 07/04: 16 days)
...
dpm_stage3 <- filter(covid, covid$date > "2020-03-01" & covid$date < "2020-03-20")
...
## Sensitivity Analysis - Reaction Stage: March 20th to March 26th
dpm_stage4 <- filter(covid, covid$date > "2020-03-19" & covid$date < "2020-03-27")

dpm_stage4 <- round(aggregate(dpm_stage4[,-2], by = list(dpm_stage4$cow), FUN = mean, na.rm = TRUE),4)

dpm_stage4$dpm4 <- ifelse(is.nan(dpm_stage4$dpm), 0, dpm_stage4$dpm)

dpm_stage4 <- select(dpm_stage4, cow, dpm4)
...
## Public Reaction Stage: March 16th to April 7th
...
gov_resp2 <- gov_resp %>%
  ...
  filter(date > "2020-03-15" & date <= "2020-04-07" & cow %in% issp16dfa$cow) %>%
  select(CountryName, date, chi)
...
gov_resp3 <- gov_resp %>%
  ...
  filter(date > "2020-03-01" & date < "2020-03-20" & cow %in% issp16dfa$cow) %>%
  select(CountryName, date, chi)
...
gov_resp4 <- gov_resp %>%
  ...
  filter(date > "2020-03-19" & date < "2020-03-27" & cow %in% issp16dfa$cow) %>%
  select(CountryName, date, chi)

gov_resp4 <- aggregate(x = gov_resp4[,-2], by = list(gov_resp4$CountryName), FUN = mean)

gov_resp4 <- gov_resp4 %>%
  mutate(
    cname = Group.1,
    cow = countrycode(cname, origin = "country.name", destination = "cown"), 
    chi4 = round(scale(chi),2)
  ) %>% 
  select(cow, chi4) %>%
  arrange(cow)
...
tab_model(m21, m22, m23, m24, m25, m26, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F,  dv.labels = c("M21", "M22","M23", "M24","M25", "M26"), pred.labels = c("Intercept", "Deaths per Million", "Current Gov. Intervention", "Discrepancy from Outbreak Phase",  "Disposable Income Inequality", "Social Spending", "GDP (k$, per capita)"))
```


#### Sensitivity Analysis: 7-day average behavior
...
```{r m3}
m3a <- lm(chi3 ~ dpm3 + gov_author, data = df)
df$chi3_p <- predict.lm(m3a, newdata = df)
df$gap2 <- df$chi3 - df$chi3_p

m31 <- lm(behave_sens ~ dpm4 + chi4, data = df)

m32 <- lm(behave_sens ~ dpm4 + gap2 + chi4, data = df)

m33 <- lm(behave_sens ~ dpm4 + gap2 + chi4 + gini_disp , data = df)

m34 <- lm(behave_sens ~ dpm4 + gap2 + chi4 + soc_spend , data = df)

m35 <- lm(behave_sens ~ dpm4 + gap2 + chi4 + gdp, data = df)

m36 <- lm(behave_sens ~ dpm4 + gap2 + chi4 + gini_disp + soc_spend + gdp, data = df)

# m36 <- lm(behave ~ dpm2*discrep3 + gini_disp + soc_spend + gdp, data = df)


tab_model(m31, m32, m33, m34, m35, m36, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F,  dv.labels = c("M31", "M32","M33", "M34","M35", "M36"), pred.labels = c("Intercept", "Deaths per Million", "Current Gov. Intervention", "Discrepancy from Outbreak Phase", "Disposable Income Inequality", "Social Spending", "GDP (k$, per capita)"))
```
