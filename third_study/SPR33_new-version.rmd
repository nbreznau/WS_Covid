...
# Create partial correlation (predicted behave partialled out from chi3)
partial <- lm(behave_reaction ~ chi3, data = df)
df$behave_p <- predict(partial, newdata = df)
df$behave_partial <- df$behave_reaction - df$behave_p

# add country ISO 3c
df$iso <- countrycode(df$cow, "cown", "iso3c")

# fix Chile as outlier for better visualization
df$gap1i <- ifelse(df$iso == "CHL", -1.4, df$gap1)
