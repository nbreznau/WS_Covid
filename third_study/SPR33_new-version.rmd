...
pacman::p_load("dplyr","countrycode","car","ggplot2","jtools","sjPlot","sjmisc","sjlabelled","tidyverse","psych","lavaan","kableExtra","ggrepel","stringi","margins","readxl","foreign", "countrycode")

```


```{r load_prep, message = F, warning = F}
# COVID Disorder Tracker
distrack <- read_xlsx(wdir("COVIDiSTRESS/coronavirus_Aug01.xlsx")) 
...
# ISSP RoG
issp06 <- foreign::read.dta(wdir("ZA4700.dta"), convert.factors = F)
issp16 <- foreign::read.dta(wdir("ZA6900_v2-0-0.dta"), convert.factors = F)

# Solt Gini
load(wdir("COVIDiSTRESS/swiid8_3.Rda"))

# NOTE: ISSP has only 32 countries 
```

```{r clean_distrack, include = F}
colnames(distrack) <- tolower(colnames(distrack))
distrack <- distrack %>%
  filter(event_type != "Strategic developments") %>%
  mutate(
    cname = countrycode(country, "country.name","iso3n", nomatch =  NULL)
  )
...
issp06 <- issp06 %>%
  mutate(
    cut_spend = car::recode(v7, '"8"=NA;"9"=NA'),
