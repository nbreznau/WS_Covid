...
## Public Reaction Stage (19/03 - 07/04)

dpm_stage2 <- filter(covid, covid$date > "2020-03-15" & covid$date <= "2020-04-07")
...
dpm_stage4 <- filter(covid, covid$date > "2020-03-19" & covid$date <= "2020-04-07")
...
## Sensitivity Analysis - Reaction Stage: March 20th to April 7th
...
ggplot(data = df2, aes(x = chi, fill = time)) +
  ...
  ylab(label = "Density") +
  scale_fill_discrete(name = "Period", labels = c("01/03-19/03","20/03 - 07/04")) +
...
tab_model(m21, m22, m23, m24, m25, m26, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F,  dv.labels = c("M21", "M22","M23", "M24","M25", "M26"), pred.labels = c("Intercept", "Deaths per Million", "Current Gov. Intervention", "Discrepancy from Outbreak Stage",  "Disposable Income Inequality", "Social Spending", "GDP (k$, per capita)"), show.std = TRUE)
```
...
tab_model(m31, m32, m33, m34, m35, m36, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F,  dv.labels = c("M31", "M32","M33", "M34","M35", "M36"), pred.labels = c("Intercept", "Deaths per Million", "Current Gov. Intervention", "Positive Discrepancy from Outbreak Stage",  "Disposable Income Inequality", "Social Spending", "GDP (k$, per capita)"), show.std = TRUE)
```
...
m41 <- lm(behave ~ dpm4 + chi4, data = df)

m42 <- lm(behave ~ dpm4 + gap2 + chi4, data = df)

m43 <- lm(behave ~ dpm4 + gap2 + chi4 + gini_disp , data = df)

m44 <- lm(behave ~ dpm4 + gap2 + chi4 + soc_spend , data = df)

m45 <- lm(behave ~ dpm4 + gap2 + chi4 + gdp, data = df)

m46 <- lm(behave ~ dpm4 + gap2 + chi4 + gini_disp + soc_spend + gdp, data = df)
