...
# later: create intervention variable as time between first death and mandatory stay home
...
```{r schooldata}
presps <- read.csv(wdir("COVIDiSTRESS/OxCGRT_timeseries_all.csv"), header = T)
...
cis_a <- select(cis_a, cow, concern_self, concern_society, dead, dead_l5, conf, conf_l5, dead_lead, dead_1st_date, Corona_concerns_1, Corona_concerns_2, Corona_concerns_3, Corona_concerns_4, Corona_concerns_5)


colnames(cis_a) <- c("cow","concern_self","concern_society", "dead", "dead_lag5", "conf", "conf_lag5","dead_lead", "dead_1st_date", "Corona1", "Corona2", "Corona3", "Corona4", "Corona5")
# add iso
...
# Days to Government Intervention, time it Took Government to Introduce Stay Home Rule

finaldf_C$intervention <- as.numeric(finaldf_C$stay_home_day - finaldf_C$dead_1st_date)

# Countries that had not introduced stay home as of June 2nd recoded to the max value 110

finaldf_C$intervention <- ifelse(finaldf_C$stay_home_day == "2020-06-02", 70, finaldf_C$intervention)

# Two countries have no deaths, VNM & GDR so rescale their values to be closer to the distribution

finaldf_C$intervention <- ifelse(finaldf_C$intervention < -20, -20, finaldf_C$intervention)
...
finaldf_C <- select(finaldf_C, country, intervention, dead_1st_date, stay_home_day, everything())
...
corC <- select(finaldf_C, Corona1, Corona2, Corona3, concern_self, Corona4, Corona5, concern_society, dead_lead_log, conf_delta_log, socpolicy, gdp, intervention)
...
cor2 <- kable(f2, digits = 2, col.names = c("Concern_Self", "Concern_Family", "Concern_Friends","Personal_Concern_Scale", "Concern_Country", "Concern_Other_Countries", "Societal_Concern_Scale","Outbreak Severity, log deaths", "New Cases, 1-week","Welfare State Strength, spend*coverage", "GDP per capita", "Government Response, days since 1st death"))
```
...
### Fig 2. Outbreak Severity and Self Concern by Country

```{r fig2, warning = F, message=F, echo=T}

ggplot(data=finaldf_Ca, aes(x=dead_lead_log, y=concern_self)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color = "grey20", linetype = "dashed") +
  geom_text_repel(aes(label = iso), size = 2.5) +
  labs(x= "Outbreak Severity (log deaths, 18 day lead)", y = "Coronavirus Concerns, Self") +
  theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())
```

### Fig 3. Outbreak Severity and Society Concern by Country

```{r fig3, warning = F, message=F, echo=T}

ggplot(data=finaldf_Ca, aes(x=dead_lead_log, y=concern_society)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color = "grey20", linetype = "dashed") +
  geom_text_repel(aes(label = iso), size = 2.5) +
...
ggplot(data=finaldf_C, aes(x=socpolicy, y=concern_self)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color = "grey20", linetype = "dashed") +
  geom_text_repel(aes(label = iso), size = 2.5) +
  labs(x= "Welfare State Strength (spending*coverage)", y = "Coronavirus Concerns, Self") +
    theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())
```
...
ggplot(data=finaldf_C, aes(x=socpolicy, y=concern)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color = "grey20", linetype = "dashed") +
  geom_text_repel(aes(label = iso), size = 2.5) +
  labs(x= "Welfare State Strength (spending*coverage)", y = "Coronavirus Concerns, Society") +
    theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())
```
...
mid <- 0.1
ggplot(data=finaldf_C, aes(x=dead_lead_log, y=concern_self, color=socpolicy)) +
  geom_point() +
  ...
  labs(x= "Outbreak Severity (log deaths, 18 day lead)", y = "Coronavirus Concerns, Self", color = "Welfare State\nStrength") +
      theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
mid <- 0.1
ggplot(data=finaldf_C, aes(x=dead_lead_log, y=concern_society, color=socpolicy)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color = "gray50", linetype = "dashed") +
  geom_text_repel(aes(label = iso), size = 2.5) +
  scale_color_gradient2(midpoint=mid, low="red", mid="gray55", high="blue", space="Lab") +
  labs(x= "Log Deaths", y = "Coronavirus Concerns, Society", color = "Welfare State\nStrength") +
      theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        legend.title = element_text(size = 9))
...
mid <- 32
ggplot(data=finaldf_C, aes(x=dead_lead_log, y=concern_self, color=gdp)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color = "gray50", linetype = "dashed") +
  geom_text_repel(aes(label = iso), size = 2.5) +
  scale_color_gradient2(midpoint=mid, low="red", mid="gray55", high="blue", space="Lab") +
...
```{r fig13, warning = F, message=F, echo=T}
mid <- 32
ggplot(data=finaldf_C, aes(x=dead_lead_log, y=concern_society, color=gdp)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color = "gray50", linetype = "dashed") +
  geom_text_repel(aes(label = iso), size = 2.5) +
  scale_color_gradient2(midpoint=mid, low="red", mid="gray55", high="blue", space="Lab") +
  labs(x= "Log Deaths", y = "Coronavirus Concerns, Society", color = "GDP\n(pc,k)") +
      theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        ...
        legend.title = element_text(size = 9))
```
#### Fig 14. Intervention and Concerns
```{r fig14, warning = F, message=F, echo=T}
mid <- 6

ggplot(data=finaldf_C, aes(x=intervention, y=concern_self, color=dead_lead_log)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color = "gray50", linetype = "dashed") +
  geom_text_repel(aes(label = iso), size = 2.5) +
  scale_color_gradient2(midpoint=mid, low="red", mid="gray55", high="blue", space="Lab") +
  labs(x= "Government Intervention, days since 1st death", y = "Coronavirus Concerns, Self", color = "Outbreak Severity\n(log deaths, 18-lead)") +
      theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        legend.title = element_text(size = 9))
...
```{r T1reg, warning = F, message=F}

m1 <- lm(concern_self ~ conf_delta_log, data = finaldf_C)
m2 <- lm(concern_self ~ dead_lead_log , data = finaldf_C)
m3 <- lm(concern_self ~ socpolicy, data = finaldf_C)
m4 <- lm(concern_self ~ gdp , data = finaldf_C)
m5 <- lm(concern_self ~ intervention, data = finaldf_C)
m6 <- lm(concern_self ~ conf_delta_log + socpolicy + gdp, data = finaldf_C)
m7 <- lm(concern_self ~ conf_delta_log + dead_lead_log + socpolicy + gdp + intervention, data = finaldf_C)

tab_model(m1, m2, m3, m4, m5, m6, m7, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"),  dv.labels = c("m1", "m2","m3","m4", "m5", "m6","m7"), pred.labels = c("New Cases, 1-week", "Outbreak Severity, log deaths (18-lead)", "Welfare State, spending*coverage", "GDP per capita, k at parity", "Intervention, days since 1st death"))
```

### Table 1a. With Outlier Countries. Regression Model, Concerns for Self


```{r T1areg, warning = F, message=F}

m1a <- lm(concern_self ~ conf_delta_log, data = finaldf_Ca)
m2a <- lm(concern_self ~ dead_lead_log , data = finaldf_Ca)
m3a <- lm(concern_self ~ socpolicy, data = finaldf_Ca)
m4a <- lm(concern_self ~ gdp , data = finaldf_Ca)
m5a <- lm(concern_self ~ intervention, data = finaldf_Ca)
m6a <- lm(concern_self ~ conf_delta_log + socpolicy + gdp, data = finaldf_Ca)
m7a <- lm(concern_self ~ conf_delta_log + dead_lead_log + socpolicy + gdp + intervention, data = finaldf_Ca)
...
### Table 2. Regression Model, Societal-Level Concerns

```{r T2reg, warning = F, message=F}

m11 <- lm(concern_society ~ conf_delta_log, data = finaldf_C)
m12 <- lm(concern_society ~ dead_lead_log , data = finaldf_C)
m13 <- lm(concern_society ~ socpolicy, data = finaldf_C)
m14 <- lm(concern_society ~ gdp , data = finaldf_C)
m15 <- lm(concern_society ~ intervention, data = finaldf_C)
m16 <- lm(concern_society ~ conf_delta_log + socpolicy + gdp, data = finaldf_C)
m17 <- lm(concern_society ~ conf_delta_log + dead_lead_log + socpolicy + gdp + intervention, data = finaldf_C)

tab_model(m11, m12, m13, m14, m15, m16, m17, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"),  dv.labels = c("m1", "m2","m3","m4", "m5", "m6","m7"), pred.labels = c("New Cases, 1-week", "Outbreak Severity, log deaths (18-lead)", "Welfare State, spending*coverage", "GDP per capita, k at parity", "Intervention, days since 1st death"))
          # file = wdir("tabs.html"))
```

### Table 2a. With Outlier Countries. Regression Model, Concerns for Self


```{r T2areg, warning = F, message=F}

m11a <- lm(concern_society ~ conf_delta_log, data = finaldf_Ca)
m12a <- lm(concern_society ~ dead_lead_log , data = finaldf_Ca)
...
m16a <- lm(concern_society ~ conf_delta_log + socpolicy + gdp, data = finaldf_Ca)
m17a <- lm(concern_society ~ conf_delta_log + dead_lead_log + socpolicy + gdp + intervention, data = finaldf_Ca)
...
```{r finalreg, echo = T}
tab_model(m7, m17, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.std = T, show.ci = F, rm.terms = c("(Intercept)"),  dv.labels = c("Self Concerns","Societal Concerns"), pred.labels = c("New Cases, 1-week", "Outbreak Severity, log deaths (18-lead)", "Welfare State, spending*coverage", "GDP per capita, k at parity", "Intervention, days since 1st death"))
```
