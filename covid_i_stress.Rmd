...
cor <- select(cis, Corona_concerns_1, Corona_concerns_2, Corona_concerns_3, concern_self, Corona_concerns_4, Corona_concerns_5, concern_society, female, age, educ_years, educ_uni)
...
cor1 <- kable(f1, digits = 2, col.names = c("Concern_Self", "Concern_Family", "Concern_Friends","Personal_Concern_Scale", "Concern_Country", "Concern_Other_Countries", "Societal_Concern_Scale","Female", "Age", "Education, years", "Education, university"))
kable_styling(cor1)
```
...
### 1.2 Johns Hopkins Covid-19 Tracker
...
deaths_long <- select(deaths_long, dead, dead_l5, date, cow)
...
# create 2.5 week lead (18 days)

deaths_long$dead_lead <- lead(deaths_long$dead, n = 18L)
...
deaths_longCL <- deaths_longCL %>%
  mutate(date = ifelse(Country == "ESP" | Country == "DEU" | Country == "RUS" | Country == "TUR" | Country == "ECU" | Country == "COL" | Country == "ZAF" | Country == "PRT" | Country == "BGD" | Country == "CHE" | Country == "UKR" | Country == "JPN" | Country == "DNK" | Country == "AFG" | Country == "CZE" | Country == "ISR" | Country == "KOR" | Country == "MAR" | Country == "GRC" | Country == "LUX" | Country == "HRV" | Country == "LTU" | Country == "ALB" | Country == "KGZ" | Country == "SVK" | Country == "NZL" | Country == "GEO" | Country == "ISL" | Country == "VNM", "2020-06-05", "2020-05-31"),
...
ggplot(data=deaths_longC, aes(x=date , y=dead_lead_log, group=Country, color=Country)) +
  ...
  geom_segment(aes(x = as.Date("2020-03-27"), y = 1, xend = as.Date("2020-03-27"), yend = 147), linetype = "dashed", color = "black", size = 0.8) +
  geom_segment(aes(x = as.Date("2020-04-30"), y = 1, xend = as.Date("2020-04-30"), yend = 147), linetype = "dashed", color = "black", size = 0.8) +
  annotate("text", x= as.Date("2020-01-22"), y= 5, 
           label="5", size=3.5, color = "gray20") +
  annotate("text", x= as.Date("2020-01-22"), y= 30, 
           label="250", size=3.5, color = "gray20") +
  annotate("text", x= as.Date("2020-01-22"), y= 55, 
           label="2k", size=3.5, color = "gray20") +
  annotate("text", x= as.Date("2020-01-22"), y= 80, 
           label="8k", size=3.5, color = "gray20") +
  annotate("text", x= as.Date("2020-01-22"), y= 105, 
           label="30k", size=3.5, color = "gray20") +
  annotate("text", x= as.Date("2020-01-22"), y= 130, 
           label="120k", size=3.5, color = "gray20") +
  ...
  geom_text(data = deaths_longCL, aes(label = Country, colour = Country, x = as.Date(date), y = dead_lead_log, hjust = -.1), size = 2) +
  theme(legend.position = "none",
        panel.background = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks = element_blank(),
        axis.text.x = element_text(angle = 45, vjust=2.15, hjust=1.5, color = "gray20"),
...
### 1.5 Maddison Data - from Book Chapter

Population of each country using wbstats
...
# add timing of policy response

presp <- presp %>%
  mutate(p_mar25 = ifelse(date == as.Date('2020-03-25') & policy == 1, 1, 0),
         p_mar25 = ifelse(date == as.Date('2020-03-25') & policy == 2, 2, p_mar25),
         p_mar25 = ifelse(date == as.Date('2020-03-25') & policy == 3, 3, p_mar25),
         p_apr4 = ifelse(date == as.Date('2020-04-04') & policy == 1, 1, 0),
         p_apr4 = ifelse(date == as.Date('2020-04-04') & policy == 2, 2, p_apr4),
         p_apr4 = ifelse(date == as.Date('2020-04-04') & policy == 3, 3, p_apr4))

presp <- select(presp, cow, p_mar25, p_apr4, date)
presp <- aggregate(presp, by = list(presp$cow), FUN = max)
```
...
cis_a <- subset(cis, date < as.Date('2020-05-01'))

cis_a <- aggregate(cis_a, by=list(cis_a$cow),
  FUN=mean, na.rm=T)
cis_a$cases <- ifelse(cis_a$cases<20, NA, cis_a$cases)
cis_a <- completeFun(cis_a, "cases")

cis_a <- select(cis_a, cow, concern_self, concern_society, dead, dead_l5, conf, conf_l5)


colnames(cis_a) <- c("cow","concern_self","concern_society", "dead", "dead_lag5", "conf", "conf_lag5")
# add iso
