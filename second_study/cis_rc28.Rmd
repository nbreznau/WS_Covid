...
m27 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp + m2r + gini_disp + I(gini_disp^2) + gdp, data = df)

tab_model(m21, m22, m23, m24, m25, m26, m27, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"), show.loglik = T, show.aic = T, dv.labels = c("M21", "M22","M23", "M24","M25","M26"), pred.labels = c("Days Since Curve Inflection", "New Case Rate", "Government Intervention", "Risk Perceptions", "Disposable Income Inequality", "Welfare State Strength", "GDP Per Capita", "Disposable Income Inequality^2"), file = wdir("second_study/Tbl2.html"))
...
m37 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp + m2r + top1 + gdp + I(top1^2), data = dft)


tab_model(m31, m32, m33, m34, m35, m36, m37, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"), show.loglik = T, show.aic = T, dv.labels = c("M31", "M32","M33", "M34","M35","M36","M37"), pred.labels = c("Days Since Curve Inflection", "New Case Rate", "Government Intervention", "Risk Perceptions", "Top 1% Income Concentration", "Welfare State Strength", "GDP Per Capita", "Top 1% Income Concentration^2"), file = wdir("second_study/Tbl2_ALT.html"))
...
```
#### Curvilinear Effect of Inequality
...
# create squared term
df$gini_disp2 <- df$gini_disp^2
# center
df$gini_disp2c <- df$gini_disp2 - mean(df$gini_disp2)

m42 <- '   # direct effect
             rate_2 ~ c*gini_disp + gini_disp2 + days_since_peak + conf_delta + gov_resp 
           # mediator
             concern_self ~ a*gini_disp + days_since_peak + conf_delta + gov_resp 
             rate_2 ~ b*concern_self 
           # indirect effect (a*b)
           ...
           # total effect
             total := c + (a*b)
         '
m42fit <- sem(m42, data = df)
summary(m42fit, fit.measures = T)

# Tbl5 <- semTable(m42fit, type = "html", print.results = F)

# save_html(Tbl5, wdir("second_study/Tbl4.html"), background = "white", libdir = wdir("second_study/"))

# knit_print.html(Tbl5)
...
```{r sem2plot}
m42ols <- lm(rate_2 ~ poly(gini_disp,2) + days_since_peak + conf_delta + gov_resp + concern_self, data = df)

m42plot <- plot_model(m42ols, type = "pred", terms = c("gini_disp"))


m42plot + labs(title = "", x = "Disposable Income Inequality", y = "Infection Rate, May 1st-20th")
```
...
m42plot +
  ...
  #geom_label_repel(aes(df, label=iso), inhereit.aes = F, size = 3, color = "blue4", segment.size = 0.1) +
  xlab("Disposable Income Inequality") +
