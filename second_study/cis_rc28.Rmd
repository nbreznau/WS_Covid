...
Data in deaths_longC taken from Johns Hopkins
...
```{r death outcomes}
infect_merge <- as.data.frame(matrix(nrow = 74, ncol = 4))
...
agg_png(file = wdir("second_study/Fig2_2nd.png"), width = 1000, height = 600, res = 144)
...
invisible(dev.off())
knitr::include_graphics(wdir("second_study/Fig2_2nd.png"))
#    
```
...
tab_model(m3r, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"), show.std = T, dv.labels = c("M2_resid"), pred.labels = c("Days Since Curve Inflection", "New Case Rate", "Intervention Severity", "Disposable Income Inequality"), file = wdir("COVIDiSTRESS/Tbl3_Study2.html"))
```
