...
Severity of outbreak should predict risk perceptions.
...
m1 <- summary(m1)
print(paste0("Adjusted r-square = ", round(m1[["r.squared"]],3)))
```
...
ggplot(df, aes(y=m1r, x=gini_disp)) +
  ...
  labs(title = "Figure 3. Pandemic Risk Perceptions and Economic Inequality", subtitle = "\'Over\' or \'under\' concern explained by disposable income gini stratification", caption = "Plotted residuals from M1 (see Table 1). Where y > 0 indicates higher risk perceptions than predicted \nfrom the outbreak severity and government response, whereas y < 0 are lower than predicted. \nIncome inequality explains about 20% of the variation in over and under-concern across countries.") +
  theme_classic() + 
  theme(
...
```{r reg2rate_2}
m10 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp , data = df)

# predicted values
df$m10p <- predict.lm(m10, df)

# residuals
df$m10r <- df$rate_2 - df$m10p

m11 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp + m2r, data = df)

m12 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp + m2r + gini_disp, data = df)

m13 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp + m2r + socpolicy, data = df)

m14 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp + m2r + gdp, data = df)

m15 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp + m2r + gini_disp + socpolicy + gdp, data = df)

tab_model(m10, m11, m12, m13, m14, m15, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"), pred.labels = c("Days Since Curve Inflection", "New Case Rate", "Government Intervention", "Risk Perceptions", "Disposable Income Inequality", "Welfare State Strength", "GDP Per Capita"))
...
```{r tbl2stdz}
tab_model(m12, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"), show.std = T)
...
```{r sem_baseline}
m20 <- '     rate_2 ~ days_since_peak + conf_delta + gov_resp + concern_self
             concern_self ~ days_since_peak + conf_delta + gov_resp 
         '
m20fit <- sem(m20, data = df)
summary(m20fit, fit.measures = T)
...
m21 <- '   # direct effect
           ...
           # total effect
             total := c + (a*b)
         '
m21fit <- sem(m21, data = df)
summary(m21fit, fit.measures = T)

```

```{r semstdz}
standardizedsolution(m21fit)
```

```{r semstdztotal}
m21_total <- '   # direct effect
             rate_2 ~ gini_disp + days_since_peak + conf_delta + gov_resp 
         '
m21_totalfit <- sem(m21_total, data = df)
summary(m21_totalfit, fit.measures = T)
standardizedsolution(m21_totalfit)
```
```{r fig4_mediation}
...
ggplot(df, aes(y=m10r, x=gini_disp)) +
