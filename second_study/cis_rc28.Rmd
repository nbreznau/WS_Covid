...
tab_model(m3r, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"), show.std = T, dv.labels = c("M2_resid"), pred.labels = c("Days Since Curve Inflection", "New Case Rate", "Intervention Severity", "Disposable Income Inequality"), file = wdir("second_study/Tbl2_resid.html"))
```
...
#### Table 2_N. Infection Increase as DV, May 1 - May 31

#### DV1. Infection Increase Ratio May
...
m25 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp + gini_disp + I(gini_disp^2), data = df)

m26 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp +  concern_self + I(concern_self^2) + gini_disp + I(gini_disp^2), data = df)



tab_model(m21, m22, m23, m24, m25, m26, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"), show.loglik = T, show.aic = T, dv.labels = c("M21", "M22","M23", "M24","M25","M26"), pred.labels = c("Days Since Curve Inflection", "New Case Rate", "Government Intervention", "Risk Perceptions", "Risk Perceptions^2", "Disposable Income Inequality", "Disposable Income Inequality^2"), file = wdir("second_study/Tbl2.html"))


```

```{r stdz_m26}
m26_beta <- lm.beta(m26)
m26_beta_concern <- m26_beta[["standardized.coefficients"]][["concern_self"]] + m26_beta[["standardized.coefficients"]][["I(concern_self^2)"]]

m26_beta_gini <- m26_beta[["standardized.coefficients"]][["gini_disp"]] + m26_beta[["standardized.coefficients"]][["I(gini_disp^2)"]]
```

#### Standardized Coefficients M26

Standardized combined effect of term + term^2

Risk Perceptions = `r print(m26_beta_concern)`
Disposable Income Inequality = `r print(m26_beta_gini`)

#### Models Predicting DV2 - Population Adjusted May Outbreak

Predicting deaths per capita with risk perceptions.

```{r reg2rate_2}
m21n <- lm(rate_3 ~ days_since_peak + conf_delta + gov_resp , data = df)

m22n <- lm(rate_3 ~ days_since_peak + conf_delta + gov_resp + concern_self, data = df)

m23n <- lm(rate_3 ~ days_since_peak + conf_delta + gov_resp + concern_self + I(concern_self^2), data = df)

m24n <- lm(rate_3 ~ days_since_peak + conf_delta + gov_resp +  gini_disp, data = df)

m25n <- lm(rate_3 ~ days_since_peak + conf_delta + gov_resp + gini_disp + I(gini_disp^2), data = df)

m26n <- lm(rate_3 ~ days_since_peak + conf_delta + gov_resp +  concern_self + I(concern_self^2) + gini_disp + I(gini_disp^2), data = df)

m27n <- lm(rate_3 ~ days_since_peak + conf_delta + gov_resp +  concern_self + I(concern_self^2) + gini_disp + I(gini_disp^2) + gdp, data = df)

tab_model(m21n, m22n, m23n, m24n, m25n, m26n, m27n, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"), show.loglik = T, show.aic = T, dv.labels = c("M21", "M22","M23", "M24","M25","M26"), pred.labels = c("Days Since Curve Inflection", "New Case Rate", "Government Intervention", "Risk Perceptions", "Risk Perceptions^2", "Disposable Income Inequality", "Disposable Income Inequality^2", "GDP per capita (k)"), file = wdir("second_study/Tbl2_DV2.html"))

# m26 <- lm(rate_3 ~ days_since_peak + conf_delta + gov_resp + concern_self + I(concern_self^2) + socpolicy, data = df)

# m27 <- lm(rate_3 ~ days_since_peak + conf_delta + gov_resp + concern_self + I(concern_self^2) + gdp, data = df)

# m28 <- lm(rate_3 ~ days_since_peak + conf_delta + gov_resp + concern_self + I(concern_self^2) + gdp, data = df)

```

```{r stdz_m26n}
m26n_beta <- lm.beta(m26n)
m26n_beta_concern <- m26n_beta[["standardized.coefficients"]][["concern_self"]] + m26n_beta[["standardized.coefficients"]][["I(concern_self^2)"]]

m26n_beta_gini <- m26n_beta[["standardized.coefficients"]][["gini_disp"]] + m26n_beta[["standardized.coefficients"]][["I(gini_disp^2)"]]
```

#### Standardized Coefficients M26n

Standardized combined effect of term + term^2

Risk Perceptions = `r print(m26n_beta_concern)`
Disposable Income Inequality = `r print(m26n_beta_gini`)
...
m35 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp + top1 + I(top1^2), data = dft)

m36 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp +  concern_self + I(concern_self^2) + top1 + I(top1^2), data = dft)

m37 <- lm(rate_2 ~ days_since_peak + conf_delta + gov_resp +  concern_self + I(concern_self^2) + top1 + I(top1^2) + gdp, data = dft)

tab_model(m31, m32, m33, m34, m35, m36, m37, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F, rm.terms = c("(Intercept)"), show.loglik = T, show.aic = T, dv.labels = c("M31", "M32","M33", "M34","M35","M36", "M37"), pred.labels = c("Days Since Curve Inflection", "New Case Rate", "Government Intervention", "Risk Perceptions", "Risk Perceptions^2", "Disposable Income Inequality", "Disposable Income Inequality^2", "GDP per capita (k)"), file = wdir("second_study/Tbl2_ALT.html"))


```

#### Standardized Coefficients M36

```{r stdz_m36}
m36_beta <- lm.beta(m36)
m36_beta_concern <- m36_beta[["standardized.coefficients"]][["concern_self"]] + m36_beta[["standardized.coefficients"]][["I(concern_self^2)"]]

m36_beta_gini <- m36_beta[["standardized.coefficients"]][["top1"]] + m36_beta[["standardized.coefficients"]][["I(top1^2)"]]
```
...
#### Table 4 (Appendix Only). Mediation

This is a combination of M2 and M23 to parse total, direct and indirect effects.

Even though we know gini^2 should not be a predictor of concern_self, we include it to make sure the dir/indir calculation is accurate
...
m43 <- '   # direct effect
           ...
           # instantaneous indir effect calc at values
             ...
             x11 := .475
             x12 := .50
           # requires fixing covariates and gini_disp2C 
             predm1 := i1+(a1*x1)+(a2*x1*x1)+w1*.30+w2*0.37+0*w3
             predm2 := i1+(a1*x2)+(a2*x2*x2)+w1*.30+w2*0.37+0*w3
             predm3 := i1+(a1*x3)+(a2*x3*x3)+w1*.30+w2*0.37+0*w3
             predm4 := i1+(a1*x4)+(a2*x4*x4)+w1*.30+w2*0.37+0*w3
             predm5 := i1+(a1*x5)+(a2*x5*x5)+w1*.30+w2*0.37+0*w3
             predm6 := i1+(a1*x6)+(a2*x6*x6)+w1*.30+w2*0.37+0*w3
             predm7 := i1+(a1*x7)+(a2*x7*x7)+w1*.30+w2*0.37+0*w3
             predm8 := i1+(a1*x8)+(a2*x8*x8)+w1*.30+w2*0.37+0*w3
             predm9 := i1+(a1*x9)+(a2*x9*x9)+w1*.30+w2*0.37+0*w3
             predm10 := i1+(a1*x10)+(a2*x10*x10)+w1*.30+w2*0.37+0*w3
             predm11 := i1+(a1*x11)+(a2*x11*x11)+w1*.30+w2*0.37+0*w3
             predm12 := i1+(a1*x12)+(a2*x12*x12)+w1*.30+w2*0.37+0*w3
...
# Tbl43 <- semTable(m43fit, type = "html", print.results = F)
# 
# save_html(Tbl43, wdir("second_study/Tbl43.html"), background = "white", libdir = wdir("second_study/"))
# 
# knit_print.html(Tbl43)

```

```{r plotting_data}
m43mat <- summary(m43fit)
...
ggplot(data = m43mat, aes(x = X_gini)) +
  geom_line(aes(y = iie_orig, color = "Instantaneous\nIndirect Effect\n "), color = "blue") +
  geom_line(aes(y = pred_YavgC, color = "Infection Increase\nMay, 2020, predicted\n "), color = "red") +
  geom_line(aes(y = pred_MC, color = "Risk Perceptions\n f(inequality)\n "), color = "green")
